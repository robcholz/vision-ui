...
vision_ui_icon_t DEFAULT_LIST_ICON = {
        ...
        .switch_header = (uint8_t*) HEADER_SWITCH_ITEM,
...
static bool vision_ui_icon_view_is_active() {
    return vision_ui_icon_view_context(vision_ui_selector_instance_get(), NULL, NULL);
}
...
static void vision_ui_notification_render() {
    ...
    if (vision_ui_notification_instance_get()->is_dismissing) {
        const bool reached_target = vision_ui_notification_instance_get()->y_notification ==
                                    vision_ui_notification_instance_get()->y_notification_trg;
        const bool removal_time_elapsed = now - vision_ui_notification_instance_get()->dismiss_start >=
    ...
    const int16_t x_notification =
            VISION_UI_SCREEN_WIDTH / 2 - vision_ui_notification_instance_get()->w_notification / 2;
    const int16_t y_notification_1 = vision_ui_notification_instance_get()->y_notification - 4;
    const int16_t y_notification_2 =
            vision_ui_notification_instance_get()->y_notification + VISION_UI_NOTIFICATION_HEIGHT;
    ...
    vision_ui_driver_box_r_draw(
            x_notification,
            y_notification_1,
            (int16_t) vision_ui_notification_instance_get()->w_notification,
    ...
    const int16_t text_x =
            x_notification + (int16_t) ((vision_ui_notification_instance_get()->w_notification - text_w) / 2);
    const int16_t text_y =
            (int16_t) (vision_ui_notification_instance_get()->y_notification + vision_ui_driver_str_height_get() - 2);
...
static void vision_ui_alert_render() {
    ...
    vision_ui_driver_box_r_draw(
            ...
            (int16_t) (vision_ui_alert_instance_get()->w_alert + 8),
    ...
    vision_ui_driver_box_r_draw(
            x_alert - 2,
            (int16_t) vision_ui_alert_instance_get()->y_alert,
            (int16_t) (vision_ui_alert_instance_get()->w_alert + 4),
    ...
    vision_ui_driver_pixel_draw(
            (int16_t) (VISION_UI_SCREEN_WIDTH / 2 + vision_ui_alert_instance_get()->w_alert / 2), y_alert - 3
    );
...
static void vision_ui_draw_list_header() {
    for (uint8_t i = 0; i < vision_ui_selector_mutable_instance_get()->selected_item->parent->child_num; i++) {
        ...
        const int16_t y_list_item = current_list_item->y_list_item + vision_ui_camera_instance_get()->y_camera +
                                    (VISION_UI_LIST_FRAME_FIXED_HEIGHT - LIST_ICON.header_height) / 2;
        // draw header
        vision_ui_driver_color_draw(1);
        ...
        if (current_list_item->type == ListItem) {
            if (vision_ui_list_icon_get_current().list_header == NULL) {
                vision_ui_driver_frame_draw(
                        header_base_x, y_list_item, LIST_ICON.header_width, LIST_ICON.header_height
                );
            } else {
                vision_ui_driver_bmp_draw(
                        header_base_x,
                        y_list_item,
                        LIST_ICON.header_width,
                        LIST_ICON.header_height,
        ...
        if (current_list_item->type == ListItem) {
            ...
            if (vision_ui_to_list_switch_item(current_list_item)->value == true) {
                ...
                if (vision_ui_list_icon_get_current().switch_on_footer == NULL) {
                    ...
                    vision_ui_driver_bmp_draw(
                            frame_x,
                            frame_y,
                            LIST_ICON.footer_width,
                            LIST_ICON.footer_height,
            ...
            if (vision_ui_to_list_slider_item(current_list_item)->is_confirmed) {
                ...
                if (vision_ui_list_icon_get_current().slider_footer == NULL) {
                    vision_ui_driver_frame_draw(
                            frame_x, frame_y, VISION_UI_LIST_SLIDER_FOOTER_WIDTH, LIST_ICON.footer_height
                    );
                } else {
                    vision_ui_driver_bmp_draw(
                            frame_x,
...
static void vision_ui_icon_view_render() {
    ...
    vision_ui_driver_color_draw(1);
    vision_ui_driver_box_draw(
            title_bar_x0, title_area_y0, VISION_UI_ICON_VIEW_TITLE_BAR_WIDTH, VISION_UI_ICON_VIEW_TITLE_AREA_HEIGHT
    );
...
static void vision_ui_selector_render() {
    const int16_t x_selector =
            (int16_t) (lrintf(vision_ui_camera_instance_get()->x_camera) +
                       VISION_UI_LIST_HEADER_TO_LEFT_DISPLAY_PADDING - VISION_UI_LIST_SELECTOR_TO_INNER_WIDGET_PADDING);
    const int16_t y_selector =
            (int16_t) lrintf(vision_ui_selector_instance_get()->y_selector + vision_ui_camera_instance_get()->y_camera);
