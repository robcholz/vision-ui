...
6. Call `vision_ui_core_init(&ui)`.
7. Call `vision_ui_render_init(&ui)`.
8. In your main loop, clear the buffer, call `vision_ui_step_render(&ui)`, then send the buffer.
...
| Function                                                              | What it does                            |
|-----------------------------------------------------------------------|-----------------------------------------|
| `vision_ui_minifont_set(vision_ui_t* ui, vision_ui_font_t font)`      | Sets the small utility font.            |
| `vision_ui_font_set(vision_ui_t* ui, vision_ui_font_t font)`          | Sets the main body font.                |
| `vision_ui_font_set_title(vision_ui_t* ui, vision_ui_font_t font)`    | Sets the title font.                    |
| `vision_ui_font_set_subtitle(vision_ui_t* ui, vision_ui_font_t font)` | Sets the subtitle font.                 |
| `vision_ui_minifont_get(const vision_ui_t* ui)`                       | Returns the current small utility font. |
| `vision_ui_font_get(const vision_ui_t* ui)`                           | Returns the current body font.          |
| `vision_ui_font_get_title(const vision_ui_t* ui)`                     | Returns the current title font.         |
| `vision_ui_font_get_subtitle(const vision_ui_t* ui)`                  | Returns the current subtitle font.      |
...
Most users can ignore this unless they need a custom allocator for embedded memory control. When a custom allocator is
installed, Vision UI uses it for constructor-created list items and for destroying those library-owned items later.
...
| Function                                                                  | What it does                                        |
|---------------------------------------------------------------------------|-----------------------------------------------------|
| `vision_ui_driver_font_set(vision_ui_t* ui, vision_ui_font_t font)`       | Activates a font in the backend.                    |
| `vision_ui_driver_font_get(const vision_ui_t* ui)`                        | Returns the currently active backend font.          |
...
| `vision_ui_driver_str_height_get(const vision_ui_t* ui)`                  | Returns the current text height.                    |
| `vision_ui_driver_font_mode_set(const vision_ui_t* ui, uint8_t mode)`     | Changes transparent versus solid text drawing mode. |
| `vision_ui_driver_font_direction_set(const vision_ui_t* ui, uint8_t dir)` | Changes text direction.                             |
...
| `vision_ui_driver_clip_window_reset(const vision_ui_t* ui)`  | Removes the clip window.                                    |
| `vision_ui_driver_buffer_clear(const vision_ui_t* ui)`       | Clears the frame buffer.                                    |
| `vision_ui_driver_buffer_send(const vision_ui_t* ui)`        | Presents the current frame buffer.                          |
| `vision_ui_driver_buffer_area_send(...)`                     | Updates only part of the buffer if the backend supports it. |
| `vision_ui_driver_buffer_pointer_get(const vision_ui_t* ui)` | Returns the raw full-size buffer pointer.                   |
